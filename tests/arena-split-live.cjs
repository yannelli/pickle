...
async function attempt() {
  ...
  let meID, ownState, peerState, food = [], seq = 0, phase = 0, splitAt = 0, peakCells = 0, sawSameCells = false, secondSplitRequested = false, thirdSplitRequested = false;
  ...
  try {
    while(Date.now()-started<70000) {
      ...
      if(me.cells.length===8&&observed?.cells.length===8&&me.cells.map(c=>c.id).sort().join()===observed.cells.map(c=>c.id).sort().join())sawSameCells=true;
      ...
      else if(phase===1&&me.cells.length===4&&me.splitCooldown===0&&!thirdSplitRequested){player.send(JSON.stringify({type:'split'}));thirdSplitRequested=true;}
      if(phase===1&&me.cells.length===8){phase=2;splitAt=ownState.time;assert.ok(me.merge>10);}
      else if(phase===2&&me.cells.length<8&&ownState.time-splitAt<11.7)throw new Error('A piece was eaten before regroup; retry in a fresh room.');
      if(phase===2&&me.cells.length===1){assert.ok(sawSameCells,'Peer must see the same eight cell IDs');assert.ok(ownState.time-splitAt>=11.7,'No early merge');assert.equal(me.merge,0);console.log(`PASS: earned mass, split 1→2→4→8, peer saw identical cells, regrouped to 1 after ${(ownState.time-splitAt).toFixed(1)}s; peak ${peakCells} cells.`);return;}
    }
