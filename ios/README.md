...
- **Consistent artwork:** native and web share six variety shapes, colors, and smiles, plus split cucumber slices and accessories. Losing a piece gives the survivor a sad face for 2.4 seconds. Threats get a red body outline; contact membranes dent without changing collision geometry. Zoom eases between sizes. The native arena and arcade use the full screen.
...
- Play opens the full-screen pet arcade: Heart hunt, Dill says, and Brine catch from the web, plus iOS-only Countertop escape (cross pantry, sink, and stove sections, jump kitchen obstacles, and build dill streaks), Cuke chop (swipe cucumbers, spare your pickle), and Jar toss (pull back and fling your pickle into a jar). Entry costs 6 energy; completed games award 10 to 34 happiness. Countertop escape keeps the saved `hop` score key and adds feedback for jumps, landings, pickups, streaks, and collisions.
