...
export function drawPickle(ctx, p, x, y, size, time = 0, own = false, screenScale = 1) {
  ...
  const colors = { classic: '#91B65A', garlic: '#B7BB70', spicy: '#D6A05B' };
  ...
  const pixel = 1 / screenScale, halfWidth = size * (cucumber ? .70 : .85);
  ctx.save(); ctx.translate(x, y); ctx.lineCap = 'round';
  if (p.dash > 0) {
    ctx.fillStyle = '#D4EB8566'; ctx.beginPath();
    ctx.ellipse(0, 0, halfWidth + 8 * pixel, size + 8 * pixel, 0, 0, Math.PI * 2); ctx.fill();
  }
  if (p.shield > 0) {
    ctx.strokeStyle = '#FFFFFFE6'; ctx.lineWidth = 2 * pixel; ctx.setLineDash([4 * pixel, 4 * pixel]);
    ctx.beginPath(); ctx.ellipse(0, 0, halfWidth + 7 * pixel, size + 7 * pixel, 0, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]);
  }
  ctx.fillStyle = '#263E311A'; ctx.beginPath();
  ctx.ellipse(pixel, 5 * pixel, halfWidth, size, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = cucumber ? '#6DA86B' : colors[p.brine] || colors.classic; ctx.strokeStyle = ink; ctx.lineWidth = (own ? 2.5 : 1.5) * pixel;
  ctx.beginPath(); ctx.roundRect(-halfWidth, -size, halfWidth * 2, size * 2, halfWidth); ctx.fill(); ctx.stroke();
  if (cucumber) {
  ...
  ctx.fillStyle = '#FFFFFF4D'; ctx.beginPath(); ctx.roundRect(-size * (cucumber ? .46 : .55), -size * .6, size * (cucumber ? .18 : .22), size * .5, 5 * pixel); ctx.fill();
  const eye = Math.max(2 * pixel, size * .075);
  for (const side of [-1, 1]) {
    ctx.fillStyle = ink; ctx.beginPath(); ctx.ellipse(side * size * .27, -size * .15 + eye * .35, eye, eye * 1.35, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = peach; ctx.beginPath(); ctx.ellipse(side * size * .47, size * .1 + eye * .75, eye * 1.7, eye * .75, 0, 0, Math.PI * 2); ctx.fill();
  }
  ctx.strokeStyle = ink; ctx.lineWidth = Math.max(pixel, size * .035); ctx.beginPath();
  ctx.moveTo(-size * .13, size * .19); ctx.quadraticCurveTo(0, size * .42, size * .13, size * .19); ctx.stroke();
  // Monochrome accessories share the native SF Symbol silhouette family.
  if (p.outfit && p.outfit !== 'original') {
    ctx.save(); ctx.translate(0, -size * 1.055); ctx.scale(size * .56 / 24, size * .45 / 24);
